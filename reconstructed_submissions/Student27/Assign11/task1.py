...
def compare(famList):
    ...
    print("Select another Orbian to compare")
...
def createBaby(famList):
    ...
    orb1 = selectOrbian(famList)
    print("Select another Orbian to be a parent")
    orb2 = selectOrbian(famList, orb1)
...
def toPasture(famList):
    ...
    # delete selected orbian from list - remove.list[position] i think
    print("Select an Orbian to send to pasture")
    goodbye = selectOrbian(famList)
    famList.remove(goodbye)
    print("Farewell Orbian " + Orbian.getName(goodbye))
def thanosSnap(famList):
    print("Uh oh. Orbian Thanos just snapped his fingers")
    thinking()
    #<<<<<<<<<<<<<< COMPLETE THE REST OF THIS FUNCTION >>>>>>>>>>>>>
    # shuffle orbian family then delete half of them
    shuffle(famList)
    toRemove = len(famList) //2
