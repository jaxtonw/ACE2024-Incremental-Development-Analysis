...
def listFamily(famList):
    ...
    for i in range(len(famList)):
        print("I am Orbian " + str(famList[i].getName().capitalize()))
...
def toPasture(famList):
    ...
    for i in range(len(famList)):
        print("\t" + str(i + 1) + ") " + famList[i].getName())
    
    removeOrbian = eval(input("Select an Orbian: "))
    print("Farewell dear " + famList[removeOrbian - 1].getName(), end= "")
    thinking()
    famList.pop(removeOrbian - 1)
...
def thanosSnap(famList):
    ...
    orbiansToDisintegrate = (len(famList)) // 2
    
    for i in range(orbiansToDisintegrate):
        famList.pop(i)
