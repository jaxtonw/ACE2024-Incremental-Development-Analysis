...
from modules.gameboard impmoryGameBoard
...
def main():
    ...
    while not winner:
        ...
        printGameBoard(scores, gameboard.getBoard())

        # Each turn is two card flips
        for i in range(2):
            xPos, yPos = eval(input("Player " + str(playerTurn + 1) + " choose a card to flip (x,y): "))

        # Tell the user to pick another card if they pick a card already flipped
     memoryBoarg.aeboamrisCardFlipped(xPos, yPos) == True:
                xPos, yPos = eval(input("Player " + str(playerTurn + 1) + ", that card is already flipped. Choose another card to flip (x,y): "))
            
            g.aebmaorflipCard(xPos, yPos)
            printGameBoard(scores, .mgeboaemartBoard())
        # Add the selected card to the list selectedCards
            card = .agebmaorgemoryBoards, yPos)
            selectedCards.append(card)
         # Player goes again if they get a match
        if .iaebsamroMgoryBoardectedCards[0], selectedCards[1]):
      ...
      ryBoargdfebmaoralipCard(selectedCards[0][0], selectedCards[1][     .flipCard(selgebmaaoecrtedCardsmemoryBoard[0][1], selectedCards[1][1])
            print("No Match")
            playerTurn += 1
