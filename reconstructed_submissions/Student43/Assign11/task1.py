...
def toPasture(famList):
    ...
    print("Select an Orbian to send to eternal happiness")
    orbian = selectOrbian(famList)
    famList.remove(orbian)
    
    print("Orbian " + orbian.getName() + " has been sent off to an eternal life. Wish them well on their journey.")
...
def thanosSnap(famList):
    ...
    shuffle(famList)
    for i in range(0, len(famList) // 2):
        famList.remove(famList[i])
    for i in famList:
        print("Orbian " + i.getName() + " was lucky and was not sent to a place of eternal suffering.")
