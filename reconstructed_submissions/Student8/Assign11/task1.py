...
def listFamily(famList):
    msg = ""  # define the message variable
    for i in famList:
        msg += "I am Orbian "
        msg += str(i.getName())
        msg += "\n"
...
def compare(famList):
    ...
    orb1 = selectOrbian(famList)  # a second parameter is not provided as a default is in place
...
def toPasture(famList):
    ...
    orbian = selectOrbian(famList)
    print("\tFarewell dear " + orbian.getName() + "...")
    famList.pop(famList.index(orbian))  # remove an Orbian based off its index value

def thanosSnap(famList):
    print("Uh oh. Orbian Thanos just snapped his fingers")
    thinking()
    
    # find and calculate an integer for half the number of Orbians and save
    leftOverNumber = round(len(famList) / 2)
    shuffle(famList)  # shuffle the Orbian objects in the list
    
    # remove half the Orbians starting from the end of the list
    for i in range(leftOverNumber):
        famList.pop(randint(0, len(famList)-1))
