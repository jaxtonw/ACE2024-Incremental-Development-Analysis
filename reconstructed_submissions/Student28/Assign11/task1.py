...
def createBaby(famList):
    ...
    orb2 = selectOrbian(famList, selected=None)
...
def toPasture(famList):
    ...
    remove = int(input("Select an Orbian: "))
    
    print("Farwell dear " + famList[remove - 1].getName(), end="")
    thinking()
    famList.pop(remove - 1)

def thanosSnap(famList):
    print("Uh oh. Orbian Thanos just snapped his fingers")
    thinking()
    #<<<<<<<<<<<<<< COMPLETE THE REST OF THIS FUNCTION >>>>>>>>>>>>>
    # Loop ensures half get snapped
    for i in range(len(famList) // 2):
        remove = randint(0, len(famList))
        famList.pop(remove - 1)
