...
# This import gives some funcitonality if your run the program from the 
#commandline. See the video in the assignment
# for instructions. Running in the terminal makes the game board play more like a 
#game instead of scrolling and
# printing a new board every time.
# You'll need to comment a line down below in the printGameBoard() function if 
#using PyCharm
import os
...
def main():
    ...
    while not winner:
        ...
        for i in range(playerCount):
            xPos, yPos = eval(input("Player " + str(playerTurn + 1) + " choose a card to flip (column, row): "))
