...
def changeColor(blobber):
    ...
    displayColor(blobber.setColor())

def feedBlobber(blobber):
    food = eval(input("Enter amount to you feed your Blobber: "))
    blobber.setRadius(food + blobber.getRadius())
    if blobber.newBlobberVolume() >= blobber.blobberVolume() * 1.1 or blobber.newBlobberVolume() <= blobber.blobberVolume() * 1.1:
        print(" Your blobber turned to dust")
    else:
        print("continue")

def blobberSpeak(blobber):
    print("My name is " + blobber.getName() + "and I am " + blobber.getColor() + "."+ "\n" + "My current happiness level is " + "vitals")
