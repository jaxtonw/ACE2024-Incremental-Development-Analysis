...
def listFamily(famList):
    for i in famList:
        print("I am Orbian " + i.getName())


def compare(famList):
    print("Choose the first Orbian")
    orb1 = selectOrbian(famList)
    print("Choose the second Orbian")
...
def createBaby(famList):
    print("Choose the first parent")
    orb1 = selectOrbian(famList)
    print("Choose the second parent")
    orb2 = selectOrbian(famList, selected=orb1)
...
def info(famList):
    ...
    print("\tand is " + str(orbian.getVolume()) + " zogs, and " + str(
        len(orbian)) + " zings")  # Volume measured in zogs, Height in zings


def toPasture(famList):
    print("Choose an Orbian to send to pasture")
    orbian = selectOrbian(famList)
    famList.remove(orbian)
    print("Goodbye to our dear friend " + orbian.getName(), end="")
    thinking()
...
def thanosSnap(famList):
    ...
    thinking()
    characterCount = len(famList)
    roundedCharacterCount = round(characterCount / 2)
    for i in range(0, roundedCharacterCount):
        famList.pop(randint(0, characterCount - 1))
        characterCount -= 1
