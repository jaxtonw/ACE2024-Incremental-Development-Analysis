...
def main():
    ...
    while not winner:
        ...
        for i in range(2):
            yPos, xPos = eval(input("Player " + str(playerTurn + 1) + " choose a card to flip: "))
            if memoryBoard.isCardFlipped(xPos,yPos) == True:
                cardFlipped = True
                while cardFlipped:
                    yPos, xPos = eval(input("That card is already flipped over. Choose another card to flip: "))
                    if memoryBoard.isCardFlipped(xPos,yPos) == False:
                        cardFlipped = False
...
main()from modules.gameboard import GameBoard
    ...
    while not winner:
        ...
        for i in range(2):
            ...
            if memoryBoard.isCardFlipped(xPos,yPos) == True:
                cardFlipped = True
                while cardFlipped:
                    xPos, yPos = eval(input("That card is already flipped over. Choose another card to flip: "))
                    if memoryBoard.isCardFlipped(xPos,yPos) == False:
                        cardFlipped = False
            selectedCards.append([xPos, yPos])
            memoryBoard.flipCard(xPos, yPos)
