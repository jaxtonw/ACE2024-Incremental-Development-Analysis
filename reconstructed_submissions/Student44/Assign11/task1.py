...
def listFamily(famList):
    ...
    for i in famList:
        # assigns orbian to the string and the str of the name
        orbian = "I am Orbian " + str(i)
...
def createBaby(famList):
    ...
    # select 1 
    orb1 = selectOrbian(famList)
    # select 2 and display already selected next to the already selected orbian
    orb2 = selectOrbian(famList, orb1)
...
def toPasture(famList):
    ...
    # selecting orbian to go
    orb = selectOrbian(famList)
    
    orbPosition = famList.index(orb)
    # pop the orbian that is at the selected orbians index
    famList.pop(orbPosition)
    # print goodbye 
    print("Farewell dear " + str(orb), end="")  
    # called thinking function
    thinking()
...
def thanosSnap(famList):
    ...
    # puts orbians in random order
    shuffle(famList)
    # determine number of orbians getting snapped
    numSnapped = len(famList) // 2
    # pop orbians off end of the list until num snapped
    for i in range(numSnapped):
        famList.pop()
