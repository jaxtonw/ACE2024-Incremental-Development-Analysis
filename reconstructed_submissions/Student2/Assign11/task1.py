...
def listFamily(famList):
    ...
    print ("Hello, my name is" )+ famList(0: )
...
def compare(famList):
    ...
    if (orb1 == orb2):
        print("\tOrbian " + orb1.getName() + " is equal to Orbian " + 
orb2.getName())
    elif (orb1 > orb2):
        print("\tOrbian " + orb1.getName() + " is bigger than Orbian " + 
orb2.getName())
    else:
        print("\tOrbian " + orb1.getName() + " is smaller than Orbian " + 
orb2.getName())
...
def info(famList):
    ...
    print("\tand is " + str(orbian.getVolume()) + " zogs, and " + str(len(orbian)) 
+ " zings") # Volume measured in zogs, Height in zings
def toPasture(famList):
    #<<<<<<<<<<<<<< COMPLETE THIS ENTIRE FUNCTION >>>>>>>>>>>>>
    orbian = selectOrbian(famList)
    print("Fairwell to dear" + orbian.getName)
    famList.remove(orbian.getName)
    # Remove this line to work on this function. It is here to prevent errors until it's defined
def thanosSnap(famList):
    print("Uh oh. Orbian Thanos just snapped his fingers")
    thinking()
    
    #<<<<<<<<<<<<<< COMPLETE THE REST OF THIS FUNCTION >>>>>>>>>>>>>## 
    famList.remove(randint.family//2)
main()
