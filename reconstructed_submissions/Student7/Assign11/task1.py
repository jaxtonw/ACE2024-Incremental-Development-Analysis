...
def listFamily(famList):
    # print all the Orbians
    for i in famList:
        print(i)

def compare(famList):
    # Select the first Orbian
...
def toPasture(famList):
    # Have user select orbian
    orb = selectOrbian(famList)
    # Print goodbye message
    print("Goodbye, " + str(orb.getName()))
    # Remove it from the list
    famList.remove(orb)
    # Print thinking to allow time to grieve the loss of the Orbian
    thinking()
...
def thanosSnap(famList):
    ...
    # shuffle the list and truncate it to half the length (leaving the odd one alive, of course)
    shuffle(famList)
