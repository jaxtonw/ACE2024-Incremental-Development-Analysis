...
def compare(famList):
    ...
    # change statements to display which one is selected
...
def toPasture(famList):
    ...
    number = famList.index(orb)
    print("So long, Orbian " + famList.pop(number).getName() + "!")

def thanosSnap(famList):
    print("Uh oh. Orbian Thanos just snapped his fingers")
    thinking()
    numberGone = int(len(famList) / 2)
    for i in range(numberGone):
        num = randint(0, len(famList))
        famList.remove(famList[num])
