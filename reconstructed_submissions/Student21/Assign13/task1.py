...
def main():
    ...
    while not winner:
        ...
        for i in range(2):
            ...
            pos = xPos, yPos
            selectedCards.append(pos)
            
            gameBoard.flipCard(xPos, yPos)
            printGameBoard(scores, gameBoard.getBoard())

        # Player goes again if they get a match
        if gameBoard.isMatch(selectedCards[0], selectedCards[1]):
            ...
            gameBoard.flipCard(selectedCards[0][0], selectedCards[1][0])
            gameBoard.flipCard(selectedCards[0][1], selectedCards[1][1])
            print("No Match")
            playerTurn += 1
